...
RSpec.describe 'index page', type: :view do
  ...
  before :each do
    ...
    Task.create!(title: 'do SWE homework', description: 'do it', start: DateTime.new(2021, 11, 7),
                 due: DateTime.new(2021, 11, 8), user_id: @user)
    Task.create!(title: 'do laundry', due: DateTime.new(2021, 11, 5), user_id: @user)
    Task.create!(title: 'finish paper', due: DateTime.new(2021, 11, 11), user_id: @user)
    Task.create!(title: 'make dinner', description: 'pasta carbonara?', due: DateTime.new(2021, 11, 10), user_id: @user)
    sign_in @user
    visit root_path
  end
