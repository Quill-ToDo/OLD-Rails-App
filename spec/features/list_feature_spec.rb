...
RSpec.describe "task list", type: :feature, js: true do
    ...
    before :each do
        Task.create!(title: 'Complete task', start:DateTime.new(2021, 11, 8), due: DateTime.new(2021, 11, 11), complete: true)
        Task.create!(title: 'Task 2', due: DateTime.new(2021, 11, 10), complete: false)
        Task.create!(title: 'Task 3', start: DateTime.new(2021, 11, 8),  due: DateTime.new(2021, 11, 21), complete: false)
    ...
    it "should not render uncompleted tick boxes as selected" do 
        expect(!find_field("Task 2", type: "checkbox", visible: false).checked? && !find_field("Task 3", type: "checkbox", visible: false).checked?)
    end
